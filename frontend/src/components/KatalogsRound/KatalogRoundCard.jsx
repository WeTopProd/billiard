...
const KatalogCard = ({ arr, setArr, ...item }) => {
  ...
  async function favorites(id) {
    setHeart(!heart);
    await axios
      .post(`http://frantsuz-shop/api/goods/${id}/favorite/`, null, {
        headers: {
    ...
    await axios.get('http://frantsuz-shop/api/goods/?is_favorited=1', {
      headers: {
        "content-type": "application/json",
        authorization: `Token ${token}`,
      }
  ...
  async function addBascket(id) {
    await axios
      .post(`http://frantsuz-shop/api/goods/${id}/shopping_cart/`, null, {
        headers: {
          "content-type": "application/json",
          authorization: `Token ${token}`,
        },
